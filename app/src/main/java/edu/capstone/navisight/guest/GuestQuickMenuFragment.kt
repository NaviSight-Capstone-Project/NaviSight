...
GuestQuickMenuFragment.kt

 */

interface GuestQuickMenuListener {
    fun onGuestQuickMenuDismissed()
    fun onGuestQuickMenuAction(actionId: Int)
}

private const val UNHIGHLIGHTED_ALPHA = 0.4f
private const val HIGHLIGHTED_ALPHA = 1.0f
private const val STT_GPS_TAG = "GuestQuickMenu STT GPS"
private const val TAG = "GuestQuickMenuFragment (Not inside CameraFragment)"

class GuestQuickMenuFragment : Fragment(R.layout.dialog_guest_quick_menu) {
    var dragListener: GuestQuickMenuListener? = null
    ...
    override fun onAttach(context: Context) {
        ...
        dragListener = parentFragment as? GuestQuickMenuListener
            ?: throw IllegalStateException("Parent must implement GuestQuickMenuListener")
    }
        ...
        when (event.action) {
            ...
            DragEvent.ACTION_DROP -> {
                ...
                if (currentHighlightedId != null) {
                    dragListener?.onGuestQuickMenuAction(currentHighlightedId!!)
                    return@OnDragListener true
                }
            ...
            DragEvent.ACTION_DRAG_ENDED -> {
                ...
                dragListener?.onGuestQuickMenuDismissed()
                return@OnDragListener true
            }
